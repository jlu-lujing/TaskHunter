...
/*
 * Port OpenCode themes into full TaskHunter theme JSON files.
 *
 * Usage:
 *   bun run themes:port:opencode --list
 *   bun run themes:port:opencode github cursor lucent-orng --force
 *   bun run themes:port:opencode aura --out-dir /tmp/taskhunter-theme-port-test --force
 ...
 *     or existing TaskHunter theme JSON files
 *   - writes complete TaskHunter light/dark theme JSON output with solid core
...
type ThemeSource = DesktopTheme | ContextThemeJson | TaskHunterTheme;
...
function isTaskHunterTheme(value: ThemeSource): value is TaskHunterTheme {
...
function syntheticTokensFromTaskHunterTheme(theme: TaskHunterTheme): ResolvedTheme {
...
async function main(): Promise<void> {
  ...
  const generated: Array<{ fileName: string; theme: TaskHunterTheme }> = [];
    ...
    if (isTaskHunterTheme(source)) {
      ...
      generated.push({
        ...
        theme: buildTheme(
          syntheticTheme,
          variant,
          syntheticTokensFromTaskHunterTheme(source),
