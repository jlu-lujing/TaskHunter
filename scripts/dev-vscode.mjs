...
const codeBin = process.env.TASKHUNTER_VSCODE_BIN || 'code';
const workspaceArg = process.argv[2] || process.env.TASKHUNTER_VSCODE_DEV_WORKSPACE || repoRoot;
const workspacePath = path.resolve(workspaceArg);

const resolveDevServerAddress = () => {
  const configured = process.env.TASKHUNTER_VSCODE_WEBVIEW_URL;
  if (!configured) {
