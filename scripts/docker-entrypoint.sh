...
# alias; TASKHUNTER_UI_PASSWORD is the canonical runtime variable.
if [ -z "${TASKHUNTER_UI_PASSWORD:-}" ] && [ -n "${UI_PASSWORD:-}" ]; then
  TASKHUNTER_UI_PASSWORD="$UI_PASSWORD"
  export TASKHUNTER_UI_PASSWORD
fi

if [ -n "${TASKHUNTER_UI_PASSWORD:-}" ]; then
...
TASKHUNTER_HOST="${TASKHUNTER_HOST:-0.0.0.0}"
export TASKHUNTER_HOST
...
if [ -n "${TASKHUNTER_UI_PASSWORD:-}" ]; then
  set -- "$@" --ui-password "$TASKHUNTER_UI_PASSWORD"
fi
"$@"
