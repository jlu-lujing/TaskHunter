...
Use this guide when running TaskHunter behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm TaskHunter works directly first.
...
- Disable gzip on the proxy if TaskHunter is already compressing responses.
...
- TaskHunter reachable directly on LAN
...
location ~ ^/api/(event|global/event|notifications/stream|taskhunter/events)$ {
    proxy_pass http://127.0.0.1:3000;
...
location = /api/taskhunter/events {
    proxy_pass http://127.0.0.1:3000;
    proxy_set_header Accept "text/event-stream";
    proxy_set_header Cache-Control "no-cache";
    proxy_buffering off;
...
- TaskHunter compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by TaskHunter, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
