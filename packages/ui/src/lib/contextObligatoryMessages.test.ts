...
describe('context obligatory message metadata', () => {
  test('preserves sibling metadata while pinning and unpinning without duplicates', () => {
    ...
    const initial = { taskhunter: { goal: { id: 'goal_1' } }, external: true };
    ...
    expect((repinned.taskhunter as Record<string, unknown>).goal).toEqual({ id: 'goal_1' });
    expect(withContextObligatoryMessage(repinned, message, false)).toEqual({
      external: true,
      taskhunter: { goal: { id: 'goal_1' }, context_obligatory_messages: [] },
    });
  });
});
