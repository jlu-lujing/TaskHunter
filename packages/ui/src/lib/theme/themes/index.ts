...
import taskhunterLightRaw from './taskhunter-light.json';
import taskhunterDarkRaw from './taskhunter-dark.json';

const flexokiLightTheme = withPrColors(flexokiLightRaw as Theme);
const flexokiDarkTheme = withPrColors(flexokiDarkRaw as Theme);
const taskhunterLightTheme = withPrColors(taskhunterLightRaw as Theme);
const taskhunterDarkTheme = withPrColors(taskhunterDarkRaw as Theme);

export const DEFAULT_LIGHT_THEME_ID = 'taskhunter-light' as const;
export const DEFAULT_DARK_THEME_ID = 'taskhunter-dark' as const;

export const themes: Theme[] = [
  taskhunterLightTheme,
  taskhunterDarkTheme,
  flexokiLightTheme,
  flexokiDarkTheme,
  ...presetThemes.filter(
    (theme) => theme.metadata.id !== 'taskhunter-light' && theme.metadata.id !== 'taskhunter-dark',
  ),
];
