...
describe('server-owned chat directories', () => {
  test('creates beneath the relocated root and uses the legacy root only for an older response', async () => {
    ...
    expect((await createChatDirectory(new Date(2026, 8, 5))).startsWith('/home/user/.config/taskhunter/chats/2026-09-05/session-')).toBe(true);
  });

  test('classifies only exact configured and actual legacy roots after warming', async () => {
    expect(isChatDirectoryPath('/work/backup/.config/taskhunter/chats/project')).toBe(false);
    await ensureChatsRootDirectory();
    expect(isChatDirectoryPath('/srv/chats/day/session-a')).toBe(true);
    expect(isChatDirectoryPath('/home/user/.config/taskhunter/chats/day/session-a')).toBe(true);
    expect(isChatDirectoryPath('/work/backup/.config/taskhunter/chats/project')).toBe(false);
    ...
    expect(isChatDirectoryForHome('/other/.config/taskhunter/chats/session-a', '/home/user')).toBe(false);
  });

  test('deletes real descendants but never shared roots, lookalikes, or traversal paths', async () => {
    for (const path of ['/srv/chats', '/home/user/.config/taskhunter/chats', '/work/backup/.config/taskhunter/chats/project', '/srv/chats/../project']) {
      await deleteChatDirectory(path);
    }
    expect(deleteRequests()).toHaveLength(0);
    await deleteChatDirectory('/srv/chats/day/session-a');
    await deleteChatDirectory('/home/user/.config/taskhunter/chats/day/session-b');
