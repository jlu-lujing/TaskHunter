...
describe('opencodeClient directory availability', () => {
  ...
  test('stats the directory through the TaskHunter filesystem route, never through OpenCode path resolution', async () => {
...
describe('opencodeClient getFilesystemHomeInfo', () => {
  ...
  test('returns the server-provided chats root', async () => {
    fsHomeResponses.push(fsHomeResponse({ home: '/Users/tester', chatsRoot: '/srv/taskhunter-chats' }));
    expect(await opencodeClient.getFilesystemHomeInfo()).toEqual({ home: '/Users/tester', chatsRoot: '/srv/taskhunter-chats' });
  });
