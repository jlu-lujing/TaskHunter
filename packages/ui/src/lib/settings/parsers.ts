 ...
 * that parse as http(s) URLs without embedded credentials, known formats.
 * Malformed entries drop silently; the whole map replaces stored providers.
 * `packages/web/server/lib/agent/types.js` is the wire-truth counterpart.
 */
export const parseEngineProviders = fromSchema(
  z.record(z.string(), z.unknown()).transform((entries) => {
    const result: Record<string, EngineProviderConfig> = {};
