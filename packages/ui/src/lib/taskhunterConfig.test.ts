...
const project = { id: 'taskhunter', path: '/workspace/taskhunter' };
...
} = await import('./taskhunterConfig');

const getConfigPath = (projectPath: string): string => (
  `${homeDirectory}/.config/taskhunter/projects/${createProjectIdFromPath(projectPath)}.json`
);
