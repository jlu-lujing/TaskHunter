 ...
 *    `window.__TASKHUNTER_HARDWARE_KEYBOARD__` at document start, then keeps it
...
declare global {
  interface Window {
    __TASKHUNTER_HARDWARE_KEYBOARD__?: boolean;
  }
}
...
const initialNativeAnswer = typeof window !== 'undefined'
  && typeof window.__TASKHUNTER_HARDWARE_KEYBOARD__ === 'boolean'
  ? window.__TASKHUNTER_HARDWARE_KEYBOARD__
  : null;
...
export const startHardwareKeyboardBridge = (): (() => void) => {
  ...
  if (typeof window.__TASKHUNTER_HARDWARE_KEYBOARD__ === 'boolean') {
    adopt(window.__TASKHUNTER_HARDWARE_KEYBOARD__);
  }
