...
const readInjectedBearerToken = (): string => {
  ...
  const injected = (window as typeof window & { __TASKHUNTER_CLIENT_TOKEN__?: string }).__TASKHUNTER_CLIENT_TOKEN__;
...
export const getRuntimeExtraHeadersSync = (): Record<string, string> => {
  ...
  if (typeof window === 'undefined') return {};
  const injected = (window as typeof window & { __TASKHUNTER_RUNTIME_HEADERS__?: Record<string, string> }).__TASKHUNTER_RUNTIME_HEADERS__;
