...
describe('taskhunter events', () => {
  ...
  test('does not open the server-only event stream in VS Code', async () => {
    ...
    const unsubscribe = subscribeTaskhunterEvents(() => undefined);
    try {
  ...
  test('dispatches externally created session events', async () => {
    ...
    const unsubscribe = subscribeTaskhunterEvents((event) => events.push(event));
  ...
  test('a connected control SSE stream clears delivered queues without reconnecting or polling', async () => {
    ...
    const source = MockEventSource.instances[0];
    try {
      source.onmessage?.({ data: JSON.stringify({ type: 'taskhunter:event-stream-ready', properties: {} }) });
      ...
      source.onmessage?.({ data: JSON.stringify({ type: 'taskhunter:message-queue.updated', properties: { revision: 2, session } }) });
      const key = getMessageQueueKey(target);
      expect(useMessageQueueStore.getState().queuedMessages[key]).toHaveLength(1);
      source.onmessage?.({ data: JSON.stringify({ type: 'taskhunter:message-queue.updated', properties: { revision: 3, session: { ...session, items: [], sendingId: null } } }) });
      ...
      unsubscribe();
      source.onmessage?.({ data: JSON.stringify({ type: 'taskhunter:message-queue.updated', properties: { revision: 4, session } }) });
      expect(useMessageQueueStore.getState().queuedMessages[key]).toBeUndefined();
    } finally {
      unsubscribe();
