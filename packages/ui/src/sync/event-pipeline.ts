...
const normalizeTaskHunterSessionStatus = (payload: Event): Event | null => {
  ...
  if (record.type !== "taskhunter:session-status") return null
  ...
  return {
    id: typeof record.id === "string" && record.id.length > 0
      ? record.id
      : `taskhunter-status-${sessionID}-${Date.now()}`,
    type: "session.status",
    properties: {
      sessionID,
      status,
...
const normalizeEventType = (payload: Event): Event => {
  const normalizedTaskHunterStatus = normalizeTaskHunterSessionStatus(payload)
  if (normalizedTaskHunterStatus) {
    return normalizedTaskHunterStatus
  }
...
export function createEventPipeline(input: EventPipelineInput): EventPipeline {
  ...
  if (typeof globalThis.window !== "undefined") {
    globalThis.window.addEventListener("taskhunter:system-resume", onSystemResume)
  ...
  const cleanup = () => {
    ...
    if (typeof globalThis.window !== "undefined") {
      globalThis.window.removeEventListener("taskhunter:system-resume", onSystemResume)
