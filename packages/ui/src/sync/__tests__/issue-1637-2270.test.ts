...
const markSessionAsTaskHunterCreatedCalls: string[] = []
...
mock.module("../session-ui-store", () => ({
  useSessionUIStore: {
    getState: () => ({
      ...
      markSessionAsTaskHunterCreated: (sessionId: string) => {
        markSessionAsTaskHunterCreatedCalls.push(sessionId)
      },
    }),
  },
}))
...
beforeEach(() => {
  ...
  markSessionAsTaskHunterCreatedCalls.length = 0
