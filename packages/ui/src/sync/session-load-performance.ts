const STORAGE_KEY = "taskhunter_session_load_perf"
...
declare global {
  interface Window {
    __taskhunterSessionLoadPerformance?: SessionLoadPerformanceState
  }
}
...
export function startSessionLoadPerformanceEvent(input: Omit<SessionLoadPerformanceEvent, "at" | "durationMs" | "outcome">) {
  ...
  return (
    ...
    const state = window.__taskhunterSessionLoadPerformance ?? { events: [] }
    ...
    window.__taskhunterSessionLoadPerformance = state
  }
}
...
const FIRST_VISIBLE_MARK = "taskhunter.chat.first_message_visible"
