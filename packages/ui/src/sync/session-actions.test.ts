...
describe("confirmed session removal", () => {
  ...
  const chatDirectory = "/home/user/.config/taskhunter/chats/2026-09-05/session-abc"
...
describe("archiving a batch through the server", () => {
  ...
  test("keeps review and btw sessions on the per-session path", async () => {
    const review = liveSession("session-review", { taskhunter: { kind: "review", originalSessionID: "session-parent" } })
    const parentWithFork = liveSession("session-parent", { taskhunter: { btwSessionID: "session-fork" } })
