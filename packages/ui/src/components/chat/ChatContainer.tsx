...
const CHAT_FORCE_SCROLL_BOTTOM_EVENT = 'taskhunter:chat-force-scroll-bottom';
...
export const ChatContainer: React.FC<ChatContainerProps> = ({
    ...
    React.useEffect(() => {
        ...
        const scopedWindow = window as typeof window & {
            __taskhunterApplyChatSettingsSync?: (payload: { allowPromptingSubagentSessions: boolean }) => void;
        };
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            if (data?.type !== 'taskhunter:chat-settings-sync'
        ...
        scopedWindow.__taskhunterApplyChatSettingsSync = applySync;
        window.addEventListener('message', handleMessage);
        parentWindow.postMessage({ type: 'taskhunter:chat-settings-request' }, window.location.origin);
        return () => {
            window.removeEventListener('message', handleMessage);
            if (scopedWindow.__taskhunterApplyChatSettingsSync === applySync) {
                delete scopedWindow.__taskhunterApplyChatSettingsSync;
            }
        };
    }, []);
