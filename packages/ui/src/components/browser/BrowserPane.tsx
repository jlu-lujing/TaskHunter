...
const ZOOM_MIN = -3;
const ZOOM_MAX = 4;
const BROWSER_PARTITION = 'persist:taskhunter-browser';
...
const WebviewBrowser: React.FC<BrowserPaneProps> = ({ initialUrl, directory, tabID }) => {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('taskhunter:zoom', handleZoom);
    return () => window.removeEventListener('taskhunter:zoom', handleZoom);
  }, [applyZoom]);
      ...
      >
        {initialSrc !== null ? (
          <webview
            ref={attachWebview}
            src={initialSrc}
            partition="persist:taskhunter-browser"
            allowpopups
            style={layout
              ? {
