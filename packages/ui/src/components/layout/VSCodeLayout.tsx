...
export const VSCodeLayout: React.FC = () => {
  ...
  const [connectionStatus, setConnectionStatus] = React.useState<'connecting' | 'connected' | 'error' | 'disconnected'>(
    () => (typeof window !== 'undefined'
      ? (window as { __TASKHUNTER_CONNECTION__?: { status?: string } }).__TASKHUNTER_CONNECTION__?.status as
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('taskhunter.setActiveSession', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('taskhunter.updateSessionEditorTitle', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    const current =
      (typeof window !== 'undefined'
        ? (window as { __TASKHUNTER_CONNECTION__?: { status?: string } }).__TASKHUNTER_CONNECTION__?.status
    ...
    window.addEventListener('taskhunter:connection-status', handler as EventListener);
    return () => window.removeEventListener('taskhunter:connection-status', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('taskhunter:navigate', handler as EventListener);
    return () => window.removeEventListener('taskhunter:navigate', handler as EventListener);
  }, []);
    ...
    const runBootstrap = async () => {
      ...
      try {
        const debugEnabled = (() => {
          ...
          try {
            return window.localStorage.getItem('taskhunter_stream_debug') === '1';
          } catch {
            return false;
          }
        })();

        if (debugEnabled) console.log('[TaskHunter][VSCode][bootstrap] attempt', { configInitialized });
        ...
        if (debugEnabled) console.log('[TaskHunter][VSCode][bootstrap] post-load', {
