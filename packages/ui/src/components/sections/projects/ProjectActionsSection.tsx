...
import {
  ...
  type TaskHunterProjectAction,  type ProjectRef,
...
type EditableProjectAction = TaskHunterProjectAction;
...
export const ProjectActionsSection: React.FC<ProjectActionsSectionProps> = ({ projectRef }) => {
  ...
  const [sharedActions, setSharedActions] = React.useState<TaskHunterProjectAction[]>([]);
  ...
  const makeActionPersonal = React.useCallback(async (action: TaskHunterProjectAction) => {
    if (isSharing) return;
    setIsSharing(true);
    try {
      const shared = await updateSharedProjectSetup(projectRef, {
                ...
                <div className="flex items-start gap-2">
                  ...
                  {action.name.trim() && action.command.trim() ? (
                    <Button
                      ...
                      title={hasChanges ? t('settings.projects.shared.actions.shareAfterSave') : t('settings.projects.shared.actions.shareTitle', { path: sharedState?.path ?? '.taskhunter/project.json' })}
