...
export const OpenCodeCliSettings: React.FC = () => {
  ...
  return (
    <SettingsSection title={t('settings.taskhunter.opencodeCli.title')}>
      <div className="space-y-0.5">
        <SettingsFieldRow
          settingsItem="sessions.opencode-binary"
          label={t('settings.taskhunter.opencodeCli.field.binaryPath')}
          info={(
            <>
              {t('settings.taskhunter.opencodeCli.tipPrefix')}
              {' '}
              <span className="font-mono">OPENCODE_BINARY</span>
              {' '}
              {t('settings.taskhunter.opencodeCli.tipMiddle')}
              {' '}
              <span className="font-mono">~/.config/taskhunter/settings.json</span>
              {'.'}
            </>
          )}
          alignEnd={false}
        ...
        >
          <Input
            ...
            placeholder={t('settings.taskhunter.opencodeCli.field.binaryPathPlaceholder')}
          ...
          <Button
            ...
            aria-label={t('settings.taskhunter.opencodeCli.actions.browseAria')}
            title={t('settings.taskhunter.opencodeCli.actions.browse')}
          >
        ...
        <SettingsInset className={SETTINGS_OPTION_STACK_CLASS}>
          {!isWindowsArm64() && (
            <SettingsCheckboxRow
              ...
              label={t('settings.taskhunter.opencodeCli.field.showUpdateNotifications')}
              ariaLabel={t('settings.taskhunter.opencodeCli.field.showUpdateNotificationsAria')}
            />
          )}
