...
export const WorktreeSectionContent: React.FC<WorktreeSectionContentProps> = ({ projectRef: projectRefProp = null, sections = 'all' }) => {
  ...
  const handleReplaceSharedCommandsChange = React.useCallback(async (next: boolean) => {
    ...
    if (!(await updateProjectSetup(projectRef, { setupWorktreeMode: next ? 'replace' : 'append' }))) {
      toast.error(t('settings.taskhunter.worktrees.setup.toast.saveFailed'));
      setReplaceSharedCommands(!next);
    }
  }, [projectRef, t]);
  ...
  const setupTooltip = (
    <SettingsInfoHint>
      {t('settings.taskhunter.worktrees.setup.tooltipPrefix')}
      {' '}
      <code className="font-mono text-xs bg-sidebar-accent/50 px-1 rounded">$ROOT_PROJECT_PATH</code>
      {' '}
      {t('settings.taskhunter.worktrees.setup.tooltipSuffix')}
    </SettingsInfoHint>
  );

  const listTooltip = (
    <SettingsInfoHint>
      {t('settings.taskhunter.worktrees.list.tooltip')}
    </SettingsInfoHint>
  );
    ...
    return (
      ...
      >
        <p className="typography-meta text-muted-foreground">
          {t('settings.taskhunter.worktrees.state.selectProject')}
        </p>
      </ProjectSettingsSubsection>
    );
  }
  ...
  if (isGitRepoLocal === false) {
    return (
      ...
      >
        <p className="typography-meta text-muted-foreground">
          {t('settings.taskhunter.worktrees.state.gitOnly')}
        </p>
      </ProjectSettingsSubsection>
    );
  }
  ...
  return (
    <>
      ...
      >
        {isLoadingCommands ? (
          <p className="typography-meta text-muted-foreground">{t('settings.taskhunter.worktrees.setup.loading')}</p>
        ) : (
          <div className={cn('space-y-2', PROJECT_SETTINGS_CONTROL_WIDTH)}>
            ...
            {setupCommands.map((command, index) => (
              <div key={index} className="flex w-full gap-2">
                <Input
                  ...
                  placeholder={t('settings.taskhunter.worktrees.setup.commandPlaceholder')}
                ...
                {command.trim() ? (
                  <Button
                    ...
                    title={commandsHaveChanges ? t('settings.projects.shared.actions.shareAfterSave') : t('settings.projects.shared.actions.shareTitle', { path: sharedConfigPath || '.taskhunter/project.json' })}
                ...
                <Button
                  ...
                  aria-label={t('settings.taskhunter.worktrees.setup.removeCommandAria')}
                >
            ...
            >
              ...
              {t('settings.taskhunter.worktrees.setup.addCommand')}
            </Button>
            <label
            ...
            >
              <Checkbox
                ...
                ariaLabel={t('settings.taskhunter.worktrees.setup.waitForCommandsAria')}
              />
              <span className={cn(
                'typography-ui-label font-normal',
                waitForSetupCommands ? 'text-foreground' : 'text-foreground/60'
              )}>
                {t('settings.taskhunter.worktrees.setup.waitForCommands')}
              </span>
            </label>
          </div>
        )}
      ...
      <ProjectSettingsSubsection
        title={t('settings.taskhunter.worktrees.list.title')}
        titleAccessory={listTooltip}
      >
        {isLoadingWorktrees ? (
          <p className="typography-meta text-muted-foreground">{t('settings.taskhunter.worktrees.list.loading')}</p>
        ) : availableWorktrees.length === 0 ? (
          <p className="typography-meta text-muted-foreground/70">
            {t('settings.taskhunter.worktrees.list.empty')}
          </p>
        ) : (
            ...
            {availableWorktrees.map((worktree) => (
              ...
              >
                <div className="min-w-0 flex-1">
                  <div className="flex min-w-0 items-center gap-2">
                    <p className="typography-meta min-w-0 truncate text-foreground">
                      {worktree.label || worktree.branch || t('settings.taskhunter.worktrees.list.detachedHead')}
                    </p>
                  </div>
                ...
                <button
                  ...
                  aria-label={t('settings.taskhunter.worktrees.list.deleteWorktreeAria', { name: worktree.branch || worktree.label || worktree.path })}
                >
