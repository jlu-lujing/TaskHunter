...
export const DesktopNetworkSettings: React.FC = () => {
  ...
  const isMacDesktop = isLocalDesktop
    ...
    && window.__TASKHUNTER_PLATFORM__ === 'darwin';
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!data) {
          throw new Error(t('settings.taskhunter.desktopNetwork.error.loadFailed'));        }
        if (cancelled) {
          return;
        }
        ...
        if (!cancelled) {
          setError(cause instanceof Error ? cause.message : t('settings.taskhunter.desktopNetwork.error.loadFailed'));
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  const handleLaunchAtLoginToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.taskhunter.desktopNetwork.error.launchAtLoginUnsupported'));
      }
      setLaunchAtLoginEnabled(status.enabled);
    } catch (cause) {
      setLaunchAtLoginEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.taskhunter.desktopNetwork.error.launchAtLoginSaveFailed'));
    } finally {
  ...
  const handleMinimizeToTrayToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status) {
        throw new Error(t('settings.taskhunter.desktopNetwork.error.minimizeToTraySaveFailed'));
      }
      if (!status.supported) {
        throw new Error(t('settings.taskhunter.desktopNetwork.error.minimizeToTrayUnsupported'));
      }
      setMinimizeToTrayEnabled(status.enabled);
    } catch (cause) {
      setMinimizeToTrayEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.taskhunter.desktopNetwork.error.minimizeToTraySaveFailed'));
    } finally {
  ...
  const handleKeepAwakeToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.taskhunter.desktopNetwork.error.keepAwakeUnsupported'));
      }
      setKeepAwakeEnabled(status.enabled);
    } catch (cause) {
      setKeepAwakeEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.taskhunter.desktopNetwork.error.keepAwakeSaveFailed'));
    } finally {
  ...
  const handleSaveAndRestart = React.useCallback(async () => {
    ...
    try {
      ...
      if (!result.ok) {
        throw new Error(t('settings.taskhunter.desktopNetwork.error.saveFailed'));      }
      ...
      if (!restarted) {
        throw new Error(t('settings.taskhunter.desktopNetwork.error.savedRestartFailed'));
      }
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : t('settings.taskhunter.desktopNetwork.error.saveFailed'));
      setIsSaving(false);
    }
  ...
  return (
    <SettingsSection title={t('settings.taskhunter.desktopNetwork.title')}>
      <div className="space-y-3">
          ...
          <div className={SETTINGS_OPTION_STACK_CLASS}>
            {launchAtLoginSupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.taskhunter.desktopNetwork.field.launchAtLogin')}
                info={t('settings.taskhunter.desktopNetwork.field.launchAtLoginDescription')}
                ariaLabel={t('settings.taskhunter.desktopNetwork.field.launchAtLoginAria')}
              />
            ) : null}
            ...
            {isMacDesktop ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.taskhunter.desktopNetwork.field.macMenuBar')}
                info={t('settings.taskhunter.desktopNetwork.field.macMenuBarDescription')}
                ariaLabel={t('settings.taskhunter.desktopNetwork.field.macMenuBarAria')}
              />
            ) : null}
            ...
            {minimizeToTraySupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.taskhunter.desktopNetwork.field.minimizeToTray')}
                info={t('settings.taskhunter.desktopNetwork.field.minimizeToTrayDescription')}
                ariaLabel={t('settings.taskhunter.desktopNetwork.field.minimizeToTrayAria')}
              />
            ) : null}
            ...
            {keepAwakeSupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.taskhunter.desktopNetwork.field.keepAwake')}
                info={t('settings.taskhunter.desktopNetwork.field.keepAwakeDescription')}
                ariaLabel={t('settings.taskhunter.desktopNetwork.field.keepAwakeAria')}
              />
            ) : null}
          </div>
        ) : null}
        ...
        <SettingsStackedField
          ...
          label={(
            <label htmlFor="desktop-ui-password">
              {t('settings.taskhunter.desktopPassword.field.password')}
            </label>
          )}
          info={t('settings.taskhunter.desktopPassword.field.passwordDescription')}
        >
          <Input
            ...
            placeholder={t(hasSavedPassword && !removePassword
              ? 'settings.taskhunter.desktopPassword.field.passwordSetPlaceholder'
              : 'settings.taskhunter.desktopPassword.field.passwordPlaceholder')}            disabled={isLoading || isSaving}
          ...
          {hasSavedPassword && !removePassword ? (
            ...
            >
              {t('settings.taskhunter.desktopPassword.actions.removePassword')}
            </Button>
          ) : null}        </SettingsStackedField>
        ...
        <div className={SETTINGS_OPTION_STACK_CLASS}>
          <SettingsCheckboxRow
            ...
            disabled={isLoading || isSaving}
            label={t('settings.taskhunter.desktopNetwork.field.allowLanAccess')}
            info={t('settings.taskhunter.desktopNetwork.field.allowLanAccessDescription')}
            description={(
              <>
                <span className="block text-[var(--status-warning)]/85">
                  {t('settings.taskhunter.desktopNetwork.field.warning')}
                </span>
                {lanRequiresPassword || lanBlockedByMissingPassword ? (
                  <span className="block text-[var(--status-warning)]/85">
                    {t('settings.taskhunter.desktopNetwork.field.passwordRequiredWarning')}
                  </span>
                ) : null}
              </>
            )}
            ariaLabel={t('settings.taskhunter.desktopNetwork.field.allowLanAccessAria')}
          />
        </div>
        ...
        {lanUrl ? (
          <div className="typography-micro text-muted-foreground/80">
            {isDirty && !savedValue
              ? t('settings.taskhunter.desktopNetwork.hint.openAfterRestart')
              : t('settings.taskhunter.desktopNetwork.hint.openNow')}
        ...
        <div className="flex justify-start py-1.5">
          <Button
            ...
            className="shrink-0 !font-normal"
          >
            {isSaving ? t('settings.common.actions.saving') : t('settings.taskhunter.desktopNetwork.actions.saveAndRestart')}
          </Button>
        </div>
      </div>
    </SettingsSection>
