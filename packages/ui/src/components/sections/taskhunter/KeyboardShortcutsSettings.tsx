...
export const KeyboardShortcutsSettings: React.FC = () => {
  ...
  const shortcutDisplay = (action: CustomizableShortcutAction): string => {
    ...
    const suffix = action.id === 'switch_session_tab'
      ? t('settings.taskhunter.keyboardShortcuts.action.switch_session_tab.suffix')
      : t('settings.taskhunter.keyboardShortcuts.action.switch_context_surface.suffix');
      ...
      {CATEGORIES.map((category, categoryIndex) => {
        ...
        return (
          <SettingsSection
            ...
            title={t(`settings.taskhunter.keyboardShortcuts.category.${category}`)}
            divider={categoryIndex !== 0}
            info={categoryIndex === 0 ? t('settings.taskhunter.keyboardShortcuts.tooltip') : undefined}
            headerAction={categoryIndex === 0 ? (
              <Button type="button" variant="outline" size="xs" className="!font-normal" onClick={() => {
                resetAllShortcutOverrides();
                persist({});
              }}>
                {t('settings.taskhunter.keyboardShortcuts.actions.resetAll')}
              </Button>
            ) : undefined}
          >
            <div className="space-y-2">
              {categoryActions.map((action) => (
                <SettingsFieldRow key={action.id} label={t(action.settingsLabelKey)}>
                  ...
                  >
                    {t('settings.taskhunter.keyboardShortcuts.actions.edit')}
                  </Button>
