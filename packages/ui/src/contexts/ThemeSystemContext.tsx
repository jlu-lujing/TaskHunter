...
export function ThemeSystemProvider({ children, defaultThemeId }: ThemeSystemProviderProps) {
  ...
  const [vscodeTheme, setVSCodeTheme] = useState<Theme | null>(() => {
    ...
    const existing = (window as unknown as { __TASKHUNTER_VSCODE_THEME__?: Theme }).__TASKHUNTER_VSCODE_THEME__;
  ...
  useEffect(() => {
    ...
    window.addEventListener('taskhunter:theme-hmr', handleThemeHmr);
    return () => window.removeEventListener('taskhunter:theme-hmr', handleThemeHmr);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('taskhunter:vscode-theme', handleThemeEvent as EventListener);
    return () => window.removeEventListener('taskhunter:vscode-theme', handleThemeEvent as EventListener);
  }, [isVSCode]);
  ...
  useEffect(() => {
    ...
    const scopedWindow = window as unknown as {
      __taskhunterApplyThemeSync?: (payload: ThemeSyncPayload) => void;
    };

    scopedWindow.__taskhunterApplyThemeSync = applyIncomingThemeSync;

    if (receivesParentThemeSync && window.parent !== window) {
      window.parent.postMessage({ type: 'taskhunter:theme-sync-request' }, window.location.origin);
    }

    return () => {
      if (scopedWindow.__taskhunterApplyThemeSync === applyIncomingThemeSync) {
        delete scopedWindow.__taskhunterApplyThemeSync;
      }
    };
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'taskhunter:theme-sync' || !data.payload) {
        return;
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('taskhunter:settings-synced', handleSettingsSynced);
    return () => window.removeEventListener('taskhunter:settings-synced', handleSettingsSynced);
  }, [receivesParentThemeSync]);
