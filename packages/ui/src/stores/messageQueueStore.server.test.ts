...
const issueMetadata = { taskhunterContext: { kind: "github-issue" as const, number: 3, title: "Bug", url: "https://x/issues/3" } }
...
describe("server-owned message queue", () => {
  ...
  test("a server item with malformed context is rejected at the boundary", async () => {
    respond = () => new Response(JSON.stringify({
      revision: 8,
      session: session([]),
      item: { ...serverItem("srv-1", "x"), context: [{ kind: "context", text: "x", metadata: { taskhunterContext: { kind: "nope" } } }] },
    }), { status: 200 })
