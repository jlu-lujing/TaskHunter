...
let persistedTaskHunterSettings: DesktopSettings | null = {};
...
mock.module('@/lib/persistence', () => ({
  ...
  // keeps every TaskHunter default unset, like the settings route used to.
  loadDesktopSettings: mock(async () => {
    settingsLoadCalls += 1;
    return loadSettingsImpl ? loadSettingsImpl() : persistedTaskHunterSettings;
  }),
}));
...
describe('useConfigStore provider persistence', () => {
  ...
  test('refreshes cached TaskHunter defaults after the default model changes', async () => {
    liveAgents = [testAgent('build')];
    liveProviderId = 'first';
    persistedTaskHunterSettings = { defaultModel: 'first/first-model' };
    ...
    persistedTaskHunterSettings = { defaultModel: 'second/second-model' };
  ...
  test('publishes configured defaults before slow catalogs finish', async () => {
    ...
    persistedTaskHunterSettings = { defaultModel: 'sidecar/chosen', defaultAgent: 'review', defaultVariant: 'high' };
  ...
  test('loads preferences before a slow OpenCode health check finishes', async () => {
    ...
    persistedTaskHunterSettings = { defaultModel: 'sidecar/chosen', defaultAgent: 'review' };
    useConfigStore.setState({ settingsDefaultsLoaded: false });
  ...
  test('does not retain another runtime defaults or directory snapshots on a failed read', async () => {
    ...
    liveAgents = [testAgent('build')];
    persistedTaskHunterSettings = { defaultModel: 'live/live-model', defaultVariant: 'high', defaultAgent: 'build' };
    ...
    persistedTaskHunterSettings = null;
  ...
  test('keeps a saved default model while a sidecar temporarily omits it', async () => {
    ...
    persistedTaskHunterSettings = { defaultModel: 'sidecar/default' };
  ...
  test('a fresh draft keeps its configured identity and thinking before discovery and after return', async () => {
    ...
    persistedTaskHunterSettings = { defaultModel: 'sidecar/chosen', defaultVariant: 'high', defaultAgent: 'build' };
    liveAgents = [testAgent('build')];
  ...
  test('does not choose Big Pickle while settings are still loading', async () => {
    ...
    persistedTaskHunterSettings = { defaultModel: 'sidecar/chosen' };
    liveAgents = [testAgent('build')];
    await useConfigStore.getState().loadAgents({ directory: DIRECTORY });
    expect(useConfigStore.getState().currentModelId).toBe('chosen');
  });
  ...
  test('does not turn an unavailable settings read into an empty default', async () => {
    ...
    persistedTaskHunterSettings = { defaultModel: 'live/live-model' };
