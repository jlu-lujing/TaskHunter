...
describe('parseConnectionPayload', () => {
  ...
  test('rejects non-connection and legacy/relay-offer payloads', () => {
    ...
    expect(parseConnectionPayload('taskhunter://connect')).toBeNull();
    expect(parseConnectionPayload('taskhunter://session/abc')).toBeNull();
    // Legacy v1 direct links are no longer accepted.
    expect(parseConnectionPayload('taskhunter://connect?v=1&server=http%3A%2F%2F192.168.1.10%3A2606&token=tok')).toBeNull();
    // Legacy relay-offer format (mode=relay + fragment) is no longer accepted.
    expect(parseConnectionPayload('taskhunter://connect?v=1&mode=relay#offer=eyJ2IjoxfQ')).toBeNull();
  });
});
...
describe('scanConnectionQr on Android', () => {
  ...
  test('falls back to string parsing when the WebView URL parser rejects the link (old Android WebView)', async () => {
    // Old Android WebViews resolve taskhunter://connect?... with hostname "" and
    ...
    const mixedCase = url.replace('taskhunter://connect', 'TaskHunter://CONNECT');
    const listeners = new Map<string, (event: { barcodes?: Array<{ rawValue?: string }> }) => void>();
    const plugin = {
      requestPermissions: mock(async () => ({ camera: 'granted' })),
