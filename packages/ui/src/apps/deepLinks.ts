/**
 * TaskHunter deep-link vocabulary — the single source of truth for the `taskhunter://`
...
const DEEP_LINK_SCHEME = 'taskhunter';
...
const segmentsOf = (url: URL): string[] => {
  // Custom-scheme URLs put the first route token in `host` (taskhunter://session/<id>),
  // but be tolerant of authority-less forms (taskhunter:/session/<id>) where it lands in
...
/**
 * Parse a raw `taskhunter://…` string into a typed intent, or `null` if it isn't a
 * recognised TaskHunter deep link. Tolerant by design: unknown routes return `null`
