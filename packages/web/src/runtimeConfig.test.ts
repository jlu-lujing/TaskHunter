...
vi.mock('@taskhunter/ui/lib/runtime-auth', () => ({
...
vi.mock('@taskhunter/ui/lib/runtime-fetch', () => ({ installRuntimeFetchBridge: vi.fn() }));
vi.mock('@taskhunter/ui/lib/runtime-switch', () => ({
  getRuntimeApiBaseUrl: vi.fn(() => ''),
  getRuntimeKey: vi.fn(() => 'local'),
  initializeRuntimeEndpoint: vi.fn(),
  switchRuntimeEndpoint: vi.fn(),
}));
vi.mock('@taskhunter/ui/lib/desktopRelayRestore', () => ({ restoreDesktopRelayRuntime: vi.fn(() => Promise.resolve()) }));
vi.mock('@taskhunter/ui/lib/runtime-url', () => ({ configureRuntimeUrlResolver: vi.fn(() => ({})) }));
vi.mock('@taskhunter/ui/lib/opencode/client', () => ({ opencodeClient: { reconnectToRuntimeBaseUrl: vi.fn() } }));
vi.mock('./api', () => ({ createWebAPIs: vi.fn() }));

import { setRuntimeBearerToken, setRuntimeExtraHeaders } from '@taskhunter/ui/lib/runtime-auth';
import { initializeRuntimeEndpoint, switchRuntimeEndpoint } from '@taskhunter/ui/lib/runtime-switch';
import { restoreDesktopRelayRuntime } from '@taskhunter/ui/lib/desktopRelayRestore';
...
const makeWindow = (search = ''): Record<string, unknown> => {
  const value: Record<string, unknown> = {
    location: { origin: 'taskhunter-ui://app', search },
    setTimeout: vi.fn(() => 1),
  };
  value.parent = value;
  return value;
...
describe('readRuntimeBootstrapConfig', () => {
  test('reads the runtime injected into the current window', () => {
    ...
    current.__TASKHUNTER_API_BASE_URL__ = ' https://remote.example.com ';
    current.__TASKHUNTER_CLIENT_TOKEN__ = ' remote-token ';
    current.__TASKHUNTER_LOCAL_ORIGIN__ = ' http://127.0.0.1:3000 ';
    current.__TASKHUNTER_RUNTIME_HEADERS__ = { 'x-taskhunter-relay': 'relay-value' };
    current.__TASKHUNTER_RELAY_HOST_ID__ = ' remote-host ';
    ...
    expect(readRuntimeBootstrapConfig()).toEqual({
      ...
      runtimeHeaders: { 'x-taskhunter-relay': 'relay-value' },
  ...
  test('does not read runtime credentials directly from a parent window', () => {
    ...
    parent.__TASKHUNTER_API_BASE_URL__ = 'https://remote.example.com';
    parent.__TASKHUNTER_CLIENT_TOKEN__ = 'remote-token';
...
describe('createConfiguredWebAPIs', () => {
  ...
  test('uses the configured desktop host id across changing SSH tunnel URLs', () => {
    const current = makeWindow();
    current.__TASKHUNTER_DESKTOP_BOOT_OUTCOME__ = {
      target: 'remote',
      status: 'ok',
      hostId: 'ssh-castle',
    ...
    current.__TASKHUNTER_API_BASE_URL__ = 'http://127.0.0.1:62545';
    current.__TASKHUNTER_LOCAL_ORIGIN__ = 'http://127.0.0.1:3901';
    installWindow(current);
