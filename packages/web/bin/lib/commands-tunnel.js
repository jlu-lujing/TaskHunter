...
async function resolveTargetInstance({
  ...
  if (options.all && requireAll) {
    if (running.length === 0) {
      throw new Error('No running TaskHunter instance found. Start one with `taskhunter serve`.');
    }
    return running;
  }
  ...
  if (options.explicitPort) {
      ...
      if (rejectDesktopRuntime) {
        ...
        if (!attachability.attachable) {
          if (attachability.reason === 'desktop') {
            throw new Error(
              `Port ${options.port} is used by TaskHunter Desktop app. Tunnel attach requires a CLI instance from \`taskhunter serve\`.`
            );
          }
          throw new Error(
            `Port ${options.port} is not an attachable TaskHunter tunnel instance. Ensure it is healthy and running TaskHunter CLI runtime.`
          );
        }
      }
      return found;
    ...
    throw new Error(`No running TaskHunter instance found on port ${options.port}.`);
  }
  ...
  if (rejectDesktopRuntime) {
    ...
    if (attachableEntries.length > 1) {
      ...
      throw new Error(`Multiple attachable TaskHunter instances found: ${ports}. Use --port <port> or --all.`);
    }
    ...
    if (sawDesktop) {
      throw new Error('Only TaskHunter Desktop instance(s) detected. Tunnel attach requires a CLI instance from `taskhunter serve`.');
    }

    throw new Error('No attachable TaskHunter instance found. Start one with `taskhunter serve`.');
  }
  ...
  throw new Error(`Multiple TaskHunter instances found: ${ports}. Use --port <port> or --all.`);
}
...
async function handleTunnelProfileSubcommand(options, action, { boldText, ensureTunnelProfilesMigrated }) {
  ...
  if (!sub) {
    ...
    if (!isQuietMode(options)) {
      ...
      clackLog.step('List profiles: `taskhunter tunnel profile list`');
      clackLog.step('Show one profile: `taskhunter tunnel profile show --name <name>`');
      clackLog.step('Add profile: `taskhunter tunnel profile add --provider cloudflare --mode managed-remote --name <name> --hostname <host> --token <token>`');
      clackLog.step('Remove profile: `taskhunter tunnel profile remove --name <name>`');
  ...
  if (sub === 'add') {
    ...
    logStatus('info', '[START_PROFILE]', `taskhunter tunnel start --profile ${added.name}`);
    clackOutro('');
    return;
  }
  ...
  throw new TunnelCliError(
    `Unknown tunnel profile subcommand '${sub}'.${hint} Use 'taskhunter tunnel help'.`,
    ...
    switch (subcommand) {
      ...
      case 'ready': {
        ...
        for (const entry of entries) {
          try {
            const { response, body } = await requestJson(entry.port, `/api/taskhunter/tunnel/check?provider=${encodeURIComponent(provider)}`);
            if (!response.ok) {
      ...
      case 'status': {
        ...
        for (const entry of entries) {
          try {
            const { response, body } = await requestJson(entry.port, '/api/taskhunter/tunnel/status');
            if (!response.ok) {
      ...
      case 'doctor': {
          ...
          for (const diagnosticsEntry of diagnosticsEntries) {
            try {
              ...
              const { response, body } = await requestJson(
                ...
                `/api/taskhunter/tunnel/doctor?${query.toString()}`,
                doctorFetchOptions,
              );
        ...
        if (desktopUnavailablePorts.length > 0) {
          clackLog.message('Only CLI instances (taskhunter serve) support tunneling.');
        }

        if (cliPorts.length === 0 && unavailablePorts.length === 0) {
          logStatus('warning', 'No running instances found', 'Start one with `taskhunter serve`.');
          clackOutro('No ports available');
          return;
        }
        if (cliPorts.length === 0) {
          logStatus('warning', 'No CLI instances available for tunneling', 'Start one with `taskhunter serve`.');
          ...
          if (isReady) {
            ...
            if (isManagedRemote && (hasPortOrOriginIssue || hasTokenIssue)) {
              troubleshootingHints.push({
                ...
                code: '[PORT_MISMATCH]',
                lines: [
                  'Cloudflare target must match the active TaskHunter CLI port.',
                  'Example: `http://127.0.0.1:<port>`',
                  'If CLI picked a different port, update Cloudflare or run `taskhunter serve --port <port>`.',
                ],
              });
            }
      ...
      case 'start': {
        ...
        if (!options.explicitPort && canPrompt(options)) {
          ...
          if (runningInstances.length > 1) {
            ...
            if (safeInstances.length === 0) {
              throw new TunnelCliError(
                'All discovered TaskHunter instance ports are browser-unsafe. Start or target a safe port (3000, 5173, 8080, or high ephemeral).',
                EXIT_CODE.USAGE_ERROR,
              );
            }
            ...
            if (attachableSafeInstances.length === 0) {
              throw new TunnelCliError(
                'No attachable TaskHunter CLI instances found on safe ports. Start one with `taskhunter serve --port 3000`.',
                EXIT_CODE.USAGE_ERROR,
              );
            }

            const selectedPort = await clackSelect({
              message: 'Select TaskHunter instance port',
        ...
        if (instance?.autoStarted && shouldRenderHumanOutput(options)) {
          logStatus(
            ...
            `logs: taskhunter logs -p ${instance.port}`,
          );
        }
        ...
        if (instance?.autoStarted) {
          ...
          healthProgress?.start(`Waiting for TaskHunter on port ${instance.port} to become healthy (up to 60s)...`);
          ...
          const healthy = await waitForServerHealth(instance.port, {
            ...
            onTick({ elapsedMs, complete }) {
              ...
              if (delta > 0) {
                ...
                healthProgress.message(`Waiting for TaskHunter health (${progressedSeconds}s / 60s)...`);
              }
          ...
          if (!healthy) {
            healthProgress?.stop('TaskHunter is still starting');
            throw new Error(
              `TaskHunter on port ${instance.port} is still starting after 60s. Startup time can vary by machine performance. ` +
              `Wait another minute, then check health with \`curl -fsS ${buildLocalUrl(instance.port, '/health')}\`. ` +
              `If health is OK, retry tunnel start with \`taskhunter tunnel start --port ${instance.port}\`. ` +
              `For diagnostics run \`taskhunter logs -p ${instance.port}\`.`
            );
          }
        ...
        if (selectedProfile && mode === 'managed-remote') {
          ...
          const { response: presetResponse, body: presetBody } = await requestJson(instance.port, '/api/taskhunter/tunnel/managed-remote-token', {
            method: 'PUT',
        ...
        let body;
        try {
          ({ response, body } = await requestJson(instance.port, '/api/taskhunter/tunnel/start', {
            method: 'POST',
            body: JSON.stringify(payload),
            timeoutMs: 60000,
          }));
        } catch (error) {
          if (error instanceof Error && /\/api\/taskhunter\/tunnel\/start/.test(error.message) && /timed out/.test(error.message)) {
            spin?.error('Tunnel start timed out');
            throw new Error(
              `Tunnel start timed out after 60s. cloudflared may still be starting; check with \`taskhunter tunnel status --port ${instance.port}\`. Run \`taskhunter logs -p ${instance.port}\` for details.`
            );
          }
          spin?.error('Tunnel start failed');
          const message = error instanceof Error ? error.message : String(error);
          throw new Error(`${message} Run \`taskhunter logs -p ${instance.port}\` for details.`);
        }
        ...
        if (!response.ok || !body?.ok) {
          ...
          const userError = isCloudflareTimeout
            ...
            : baseError;
          throw new Error(`${userError} Run \`taskhunter logs -p ${instance.port}\` for details.`);
        }
        ...
        if (isJsonMode(options)) {
          ...
          const optionalTips = [
            { line: 'Check status', detail: 'taskhunter tunnel status' },
            { line: 'Stop tunnel', detail: 'taskhunter tunnel stop' },
          ...
          if (!selectedProfile && mode === 'managed-remote' && typeof hostname === 'string' && hostname.trim().length > 0) {
            ...
            optionalTips.push({ line: 'Start from saved profile', detail: 'taskhunter tunnel start --profile <name>' });
          }
      ...
      case 'stop': {
        ...
        for (const entry of entries) {
          ...
          try {
            const { response, body } = await requestJson(entry.port, '/api/taskhunter/tunnel/stop', {
              method: 'POST',
            });
            if (!response.ok) {
      ...
      default: {
        ...
        const hint = suggestion ? ` Did you mean '${suggestion}'?` : '';
        throw new TunnelCliError(
          `Unknown tunnel subcommand '${subcommand}'.${hint} Use 'taskhunter tunnel help'.`,
          EXIT_CODE.USAGE_ERROR
        );
      }
    }
