...
async function getPidFilePath(port) {
  return path.join(getRunDir(), `taskhunter-${port}.pid`);
}

async function getInstanceFilePath(port) {
  return path.join(getRunDir(), `taskhunter-${port}.json`);
}
...
// that PID to an unrelated process — see isTaskhunterProcessRunning.
...
function isTaskhunterCmdline(cmdline) {
  if (typeof cmdline !== 'string' || cmdline.length === 0) {
    return false;
  }
  // Every install path contains the "taskhunter" segment — the npm package
  // (@taskhunter/web) and the source checkout both do, for the foreground
  ...
  return cmdline.toLowerCase().includes('taskhunter');
}

// Liveness + identity — "is the TaskHunter instance recorded in a pid file
...
function isTaskhunterProcessRunning(pid) {
  const state = getTaskhunterProcessState(pid);
  return state === 'matched' || state === 'unknown';
}

function getTaskhunterProcessState(pid, options = {}) {
  ...
  return isTaskhunterCmdline(cmdline) ? 'matched' : 'mismatched';
}

function hasTaskhunterRuntimeInfo(info) {
