...
async function stopCommand(options) {
    ...
    if (options.explicitPort) {
      ...
      if (runningInstances.length === 0) {
        ...
        if (showOutput) {
          logStatus('info', `no TaskHunter instance found on port ${options.port}`);
      ...
      if (explicitInstance.runtime === 'desktop') {
        ...
        if (isJsonMode(options)) {
          printJson({ stoppedCount: 0, results: jsonResults, messages: [{ level: 'warning', code: 'DESKTOP_MANAGED_PORT', message: `Port ${options.port} is managed by TaskHunter Desktop and cannot be stopped with this command.` }] });
        }
        if (showOutput) {
          logStatus('warning', `port ${options.port} is managed by TaskHunter Desktop`, 'cannot be stopped with this command');
      ...
      if (explicitInstance.source === 'probe') {
        ...
        if (showOutput && !unmanagedStopSpin) {
          logStatus('info', `found unmanaged TaskHunter instance on port ${options.port}`, 'attempting shutdown');
        }
        unmanagedStopSpin?.start(`Stopping unmanaged TaskHunter on port ${options.port}...`);
        ...
        if (stopped) {
          unmanagedStopSpin?.stop(`Stopped unmanaged TaskHunter on port ${options.port}`);
          jsonResults.push({ port: options.port, runtime: 'unmanaged', stopped: true });
          if (isJsonMode(options)) {
            printJson({ stoppedCount: 1, results: jsonResults });
          }
          if (showOutput && !unmanagedStopSpin) {
            logStatus('success', `stopped TaskHunter on port ${options.port}`);
          ...
          unmanagedStopSpin?.error(`Could not stop TaskHunter on port ${options.port}`);
          ...
          if (isJsonMode(options)) {
            printJson({
              ...
              stoppedCount: 0,
              results: jsonResults,
              messages: [{ level: 'error', code: 'STOP_FAILED', message: `Could not stop TaskHunter on port ${options.port}.` }],
            });
          }
          if (showOutput && !unmanagedStopSpin) {
            logStatus('error', `could not stop TaskHunter on port ${options.port}`);
            finish('failed');
          }
          printQuietStopResults();
        }
      ...
      if (explicitInstance.source === 'registry-unconfirmed') {
        ...
        if (showOutput && !unconfirmedStopSpin) {
          logStatus('info', `found unconfirmed TaskHunter pid ${explicitInstance.pid} on port ${options.port}`, 'HTTP shutdown endpoint is unreachable; stopping by PID');
        }
        unconfirmedStopSpin?.start(`Stopping unconfirmed TaskHunter on port ${options.port}...`);
        ...
        if (stopped || !isProcessRunning(explicitInstance.pid)) {
          ...
          unconfirmedStopSpin?.stop(`Stopped TaskHunter PID ${explicitInstance.pid}`);
        ...
        unconfirmedStopSpin?.error(`Could not stop TaskHunter PID ${explicitInstance.pid}`);
        ...
        if (isJsonMode(options)) {
          printJson({
            ...
            stoppedCount: 0,
            results: jsonResults,
            messages: [{ level: 'error', code: 'STOP_FAILED', message: `Could not stop TaskHunter PID ${explicitInstance.pid}.` }],
          });
        }
        if (showOutput && !unconfirmedStopSpin) {
      ...
      if (showOutput) {
        logStatus('info', 'No running TaskHunter instances found');
        finish('nothing to stop');
      }
      printQuietStopResults();
      return;
    ...
    for (const instance of runningInstances) {
      ...
      stopSpin?.start(`Stopping TaskHunter on port ${instance.port}...`);
      try {
        ...
        stopSpin?.stop(`Stopped TaskHunter on port ${instance.port}`);
        jsonResults.push({ port: instance.port, pid: instance.pid, stopped: true });
        if (showOutput && !stopSpin) {
          logStatus('success', `stopped port ${instance.port}`);
        }
      } catch (error) {
        stopSpin?.error(`Failed to stop TaskHunter on port ${instance.port}`);
...
async function restartCommand(options, serveCommand) {
    ...
    if (runningInstances.length === 0) {
      ...
      if (showOutput) {
        logStatus('info', 'No running TaskHunter instances to restart');
    ...
    for (const instance of runningInstances) {
      if (instance.runtime === 'desktop') {
        const message = `Port ${instance.port} is managed by TaskHunter Desktop and cannot be restarted with this command.`;
        if (isJsonMode(options)) {
          printJson({
            status: 'warning',
            restartedCount: 0,
        ...
        if (showOutput) {
          logStatus('warning', `port ${instance.port} is managed by TaskHunter Desktop`, 'cannot be restarted with this command');
      ...
      restartSpin?.start(`Restarting TaskHunter on port ${instance.port}...`);
      try {
        await runStop({
          explicitPort: true,
          port: instance.port,
        ...
        restartSpin?.stop(`Restarted TaskHunter on port ${restartedPort}`);
        if (showOutput && !restartSpin) {
          logStatus('success', `port ${restartedPort} restarted`, `mode: ${launchMode}`);
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        restartSpin?.error(`Failed to restart TaskHunter on port ${instance.port}`);
        if (showOutput && !restartSpin) {
