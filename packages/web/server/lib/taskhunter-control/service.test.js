...
import { createTaskHunterControlService } from './service.js';
...
const createService = (overrides = {}) => {
  ...
  const service = createTaskHunterControlService({
...
describe('TaskHunter control service', () => {
  ...
  it('rejects actions outside the fixed contract', async () => {
    const { service } = createService();
    await expect(service.execute('session.delete')).rejects.toThrow('Unsupported TaskHunter action');
  });
});
...
describe('browser capture', () => {
  ...
  it('saves the image beside the code and hands back a path the answer can use', async () => {
    ...
    expect(result.path.startsWith('.taskhunter/screenshots/after-fix-')).toBe(true);
