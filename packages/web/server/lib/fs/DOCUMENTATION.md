...
- Workspace checks accept, besides the active workspace and its worktrees, the **managed roots**: the TaskHunter config root and the managed chats root (`managedChatsRoot` dependency; `TASKHUNTER_CHATS_DIR` upstream, default `<config root>/chats`). Chat worktrees may legitimately live outside every project workspace.
