...
const registerExec = ({ spawn }) => {
  ...
  registerFsRoutes(app, {
    ...
    taskhunterUserConfigRoot: '/home/user/.config',
  });
...
describe('fs upload', () => {
  ...
  it('cleans up a partial temp file when the configured streaming limit is exceeded', async () => {
    const previous = process.env.TASKHUNTER_FS_UPLOAD_MAX_BYTES;
    process.env.TASKHUNTER_FS_UPLOAD_MAX_BYTES = '5';
    const write = vi.fn(async (_buffer, _offset, length) => ({ bytesWritten: length }));
    const fsPromises = {
      open: vi.fn(async () => ({ write, close: vi.fn(async () => undefined) })),
      link: vi.fn(async () => undefined),
    ...
    try {
      ...
      if (previous === undefined) delete process.env.TASKHUNTER_FS_UPLOAD_MAX_BYTES;
      else process.env.TASKHUNTER_FS_UPLOAD_MAX_BYTES = previous;
    }
  });
...
describe('fs exec git-read cache', () => {
  beforeEach(() => {
    delete process.env.TASKHUNTER_GIT_READ_CACHE_TTL_MS;
  });
  ...
  it('disables caching when TTL is 0', async () => {
    process.env.TASKHUNTER_GIT_READ_CACHE_TTL_MS = '0';
    const command = 'git rev-parse --absolute-git-dir';
    const { spawn, calls } = createSpawn({ stdoutByCommand: { [command]: '/repo/.git\n' } });
    const handler = registerExec({ spawn });
...
describe('fs stat directory error handling', () => {
  it('loads in Node without workspace node_modules, as packaged desktop does', async () => {
    const directory = await mkdtemp(path.join(tmpdir(), 'taskhunter-fs-import-'));
    try {
...
describe('fs managed chats root', () => {
  const registerWithChatsRoot = ({ managedChatsRoot, fsPromises = {} } = {}) => {
    ...
    registerFsRoutes(app, {
      ...
      buildAugmentedPath: () => '/usr/bin',
      resolveGitBinaryForSpawn: () => 'git',
      taskhunterUserConfigRoot: '/home/user/.config/taskhunter',
      managedChatsRoot,
    });
    return {
  ...
  it('exposes the default chats root next to the home directory', async () => {
    ...
    expect(res.body.chatsRoot).toBe('/home/user/.config/taskhunter/chats');
  });

  it('exposes a relocated chats root when TASKHUNTER_CHATS_DIR is configured upstream', async () => {
    const { home } = registerWithChatsRoot({ managedChatsRoot: '/srv/taskhunter-chats' });

    const res = createMockResponse();
    await home(undefined, res);

    expect(res.statusCode).toBe(200);
    expect(res.body.chatsRoot).toBe('/srv/taskhunter-chats');
  });

  it('allows mkdir inside the relocated chats root outside the active workspace', async () => {
    const mkdirCalls = [];
    const { mkdir } = registerWithChatsRoot({
      managedChatsRoot: '/srv/taskhunter-chats',
      fsPromises: {
    ...
    const res = createMockResponse();
    await mkdir({ body: { path: '/srv/taskhunter-chats/2026-08-25/session-a' } }, res);

    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
    expect(mkdirCalls).toEqual(['/srv/taskhunter-chats/2026-08-25/session-a']);
  });

  it('still rejects mkdir outside the workspace and all managed roots', async () => {
    const { mkdir } = registerWithChatsRoot({ managedChatsRoot: '/srv/taskhunter-chats' });
