...
const previousDataDirectory = process.env.TASKHUNTER_DATA_DIR;
const temporaryDataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'taskhunter-opencode-go-'));
process.env.TASKHUNTER_DATA_DIR = temporaryDataDirectory;
...
afterAll(() => {
  if (previousDataDirectory === undefined) delete process.env.TASKHUNTER_DATA_DIR;
  else process.env.TASKHUNTER_DATA_DIR = previousDataDirectory;
