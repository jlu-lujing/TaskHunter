...
4. The **relay** (`taskhunter-website/apps/api`, Cloudflare Worker) verifies the signature +
...
- **Relay (default):** server has no Apple key; `TASKHUNTER_PUSH_RELAY_URL` defaults to
  `https://api.openchamber.dev/v1/push/send` (register URL is derived as `…/register-token`).
- **Direct (fallback):** set `TASKHUNTER_PUSH_RELAY_DISABLED=true` + `TASKHUNTER_APNS_KEY_ID/
  TEAM_ID/P8` to sign+send from the server itself (HTTP/2 + ES256 JWT); no relay binding needed.

## Config

Server (`apns-runtime.js`):
- `TASKHUNTER_PUSH_RELAY_URL` (default the public relay), `TASKHUNTER_APNS_ENVIRONMENT`
...
- Direct fallback: `TASKHUNTER_APNS_KEY_ID`, `TASKHUNTER_APNS_TEAM_ID`, `TASKHUNTER_APNS_P8`
  (or `_P8_PATH`), `TASKHUNTER_APNS_BUNDLE_ID`, `TASKHUNTER_PUSH_RELAY_DISABLED=true`.
...
1. Apple **Keys** (not Certificates) → create an **APNs Auth Key** (`.p8`) → Key ID + Team ID;
   enable **Push Notifications** on App ID `com.taskhunter.app`.
2. In the **taskhunter-website** repo → Actions secrets: `APNS_P8` (PEM), `APNS_KEY_ID`,
