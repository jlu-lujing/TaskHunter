...
//     for self-hosters who set TASKHUNTER_APNS_* and TASKHUNTER_PUSH_RELAY_DISABLED=true.
...
const DEFAULT_BUNDLE_ID = 'com.taskhunter.app';
...
export const createApnsRuntime = (deps) => {
  ...
  const resolveApnsConfig = async () => {
    let keyId = trimmedEnv('TASKHUNTER_APNS_KEY_ID');
    let teamId = trimmedEnv('TASKHUNTER_APNS_TEAM_ID');
    let bundleId = trimmedEnv('TASKHUNTER_APNS_BUNDLE_ID');
    let environment = (trimmedEnv('TASKHUNTER_APNS_ENVIRONMENT') || '').toLowerCase();
    let p8 = normalizePem(process.env.TASKHUNTER_APNS_P8 || '');

    const p8Path = trimmedEnv('TASKHUNTER_APNS_P8_PATH');
    if (!p8 && p8Path) {
      try {
        p8 = (await fsPromises.readFile(p8Path, 'utf8')).trim();
      } catch (error) {
        console.warn('[APNs] Failed to read TASKHUNTER_APNS_P8_PATH:', error?.message ?? error);
      }
    }
  ...
  // is the fallback for self-hosters who set TASKHUNTER_APNS_* and disable the relay.
  const resolveRelayConfig = () => {
    if (trimmedEnv('TASKHUNTER_PUSH_RELAY_DISABLED') === 'true') return null;
    const url = trimmedEnv('TASKHUNTER_PUSH_RELAY_URL') || DEFAULT_RELAY_URL;
    const override = (trimmedEnv('TASKHUNTER_APNS_ENVIRONMENT') || '').toLowerCase();
    return {
      url,
      registerUrl: url.replace(/\/send$/, '/register-token'),
      // Explicit TASKHUNTER_APNS_ENVIRONMENT forces every send to that environment; when
  ...
  const sendViaRelay = async (deviceTokens, payload, relay, environment) => {
    ...
    const title = typeof payload?.title === 'string' && payload.title.length > 0 ? payload.title : 'TaskHunter';
    const { privateKey, publicJwk } = await getOrCreateRelayKeypair();
    const ts = Date.now();
  ...
  const sendViaDirectApns = async (tokenGroups, payload) => {
    ...
    if (!config) {
      if (!warnedUnconfigured) {
        ...
        console.warn(
          '[APNs] Relay disabled and no direct config; set TASKHUNTER_APNS_KEY_ID / TASKHUNTER_APNS_TEAM_ID / TASKHUNTER_APNS_P8 for direct send.',
        );
      }
      return;
    }
