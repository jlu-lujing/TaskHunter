...
This module injects an opt-in OpenCode plugin only when TaskHunter launches
...
1. Settings persist `optimizeSystemPrompt` in TaskHunter's `settings.json`.
2. The setting is applied when managed OpenCode restarts.
3. The runtime materializes the plugin under
   `<taskhunter-data-dir>/system-prompt/` and appends its `file://` URL to
