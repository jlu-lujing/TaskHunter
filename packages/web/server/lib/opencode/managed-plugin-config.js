...
/**
 * Append a managed TaskHunter plugin to the `plugin` list of an
...
export const appendManagedPlugin = (rawConfig, pluginUrl, purpose) => {
  ...
  if (errors.length > 0 || !isJsonObject(parsed)) {
    throw new Error(`OPENCODE_CONFIG_CONTENT must contain a valid JSON object before TaskHunter can inject its ${purpose}`);
  }
  if (parsed.plugin !== undefined && !Array.isArray(parsed.plugin)) {
    throw new Error(`OPENCODE_CONFIG_CONTENT plugin must be an array before TaskHunter can inject its ${purpose}`);
  }
