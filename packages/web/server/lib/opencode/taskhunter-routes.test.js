...
const createApp = ({ environment = {}, storedOptions = {}, desktopUpdater, platform = 'linux', execPath = '/usr/bin/node' } = {}) => {
  ...
  const dependencies = {
    ...
    __dirname: '/opt/taskhunter/server',
    taskhunterDataDir: '/tmp/taskhunter',
...
beforeEach(() => {
  ...
  packageManager.getUpdateCommand.mockReturnValue('npm install -g @taskhunter/web@latest');
});
...
describe('TaskHunter foreground update route', () => {  it('rejects a foreground update when the server is not owned by systemd', async () => {
  ...
  it('rejects an unsafe systemd unit override before starting an update job', async () => {
    const { app } = createApp({
      environment: {
        ...
        TASKHUNTER_SYSTEMD_UNIT: 'taskhunter.service; rm -rf /',
      },
    });
  ...
  it('queues the install in a transient systemd unit and returns its job identifier', async () => {
    ...
    const { app } = createApp({
      environment: {
        INVOCATION_ID: 'systemd-invocation',
        TASKHUNTER_SYSTEMD_UNIT: 'taskhunter@wsl.service',
    ...
    await request(app)
      ...
      .expect(200, {
        ...
        jobId: 'taskhunter-update-1700000000000',
        logPath: 'journalctl --user-unit taskhunter-update-1700000000000.service',
      });

    expect(childProcess.spawnSync).toHaveBeenCalledWith('systemd-run', [
      '--user',
      '--unit=taskhunter-update-1700000000000',
      '--collect',
      '--service-type=exec',
      '--setenv=PATH=/home/syu/.npm-global/bin:/usr/bin:/bin',
      '/bin/sh',
      '-c',
      "set -eu\nnpm install -g @taskhunter/web@latest\nsystemctl --user restart 'taskhunter@wsl.service'",
    ], {
      encoding: 'utf8',
...
describe('TaskHunter web update route on Windows', () => {
  it('runs the install-and-restart script from a batch file instead of a cmd.exe /c argument', async () => {
    ...
    const scriptPath = path.join('/tmp/taskhunter', 'update-install.cmd');
    ...
    expect(lines).toContain('echo restartCommand=^("C:\\Program Files\\nodejs\\node.exe" "/opt/taskhunter/bin/cli.js" serve --port 7897 --ui-password "pa%%ss"^) ^|^| ^(taskhunter serve --port 7897 --ui-password "pa%%ss"^)');
    // A .cmd shim (npm, pnpm, yarn) must be `call`ed or the script ends there.
    expect(lines).toContain('call npm install -g @taskhunter/web@latest');
  ...
  it('answers 500 and keeps the server up when the batch file cannot be written', async () => {
    ...
    const response = await request(app).post('/api/taskhunter/update-install').expect(500);
    await new Promise((resolve) => setTimeout(resolve, 1300));
