        ...
        try {
          ...
          if (typeof process.send === 'function') {
            if (!process.connected) {
              throw new Error('TaskHunter startup IPC channel disconnected before ready notification');
            }

            await new Promise((resolveReadyNotification, rejectReadyNotification) => {
              try {
                process.send({ type: 'taskhunter:ready', port: activePort }, (error) => {
                  if (error) {
          ...
          console.log(`TaskHunter server listening on ${bindHost}:${activePort}`);
