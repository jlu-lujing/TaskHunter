export const registerProjectIconRoutes = (app, dependencies) => {
  ...
  const projectIconsDirPath = path.join(taskhunterDataDir, 'project-icons');
  ...
  const applyProjectIconSvgTheme = (svgMarkup, themeVariant, iconColor) => {
    ...
    const overrideStyle = `<style data-taskhunter-theme-icon="1">:root{color:${color}!important;}</style>`;
