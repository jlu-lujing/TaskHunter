...
describe('startup performance diagnostics', () => {
  const previousValue = process.env.TASKHUNTER_STARTUP_PERF;

  afterEach(() => {
    if (previousValue === undefined) delete process.env.TASKHUNTER_STARTUP_PERF;
    else process.env.TASKHUNTER_STARTUP_PERF = previousValue;
    vi.restoreAllMocks();
  });

  it('is disabled by default', () => {
    delete process.env.TASKHUNTER_STARTUP_PERF;
  ...
  it('records only approved labels and numeric metadata', () => {
    process.env.TASKHUNTER_STARTUP_PERF = '1';
    const info = vi.spyOn(console, 'info').mockImplementation(() => {});
  ...
  it('rejects unknown phases and invalid field values', () => {
    process.env.TASKHUNTER_STARTUP_PERF = 'true';
    const info = vi.spyOn(console, 'info').mockImplementation(() => {});
