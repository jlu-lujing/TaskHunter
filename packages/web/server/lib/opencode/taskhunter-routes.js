...
function resolveSystemdServiceUnit(environment) {
  ...
  const configuredUnit = typeof environment.TASKHUNTER_SYSTEMD_UNIT === 'string'
    ? environment.TASKHUNTER_SYSTEMD_UNIT.trim()
    : '';
  const unit = configuredUnit || 'taskhunter.service';
...
const buildWindowsUpdateScript = ({ logPreamble, updateCmd, restartCmd }) => [
  ...
  'if %ERRORLEVEL% EQU 0 (',
  '  echo Update successful, restarting TaskHunter...',
  `  ${restartCmd ? restartCmd.replace(/%/g, '%%') : 'echo Service manager will restart TaskHunter.'}`,
  ') else (',
  '  echo Update failed',
  ')',
...
export const registerTaskHunterRoutes = (app, dependencies) => {
  const {
    fs,
    path,
    process,
  ...
  app.get('/api/taskhunter/update-check', async (req, res) => {
    try {
      ...
      let updateInfo;
      if (process.env.TASKHUNTER_RUNTIME === 'desktop' && updateRequest.appType === 'web') {
  ...
  app.post('/api/taskhunter/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(taskhunterDataDir, 'run', `taskhunter-${currentPort}.json`);
      ...
      if (isForegroundService) {
        if (!systemdServiceUnit) {
          return res.status(409).json({
            error: 'Foreground servers must be updated by their service manager. Set TASKHUNTER_SYSTEMD_UNIT when running under systemd, or run taskhunter update and restart the service.',
          });
        }

        const updateJobName = `taskhunter-update-${Date.now()}`;
        ...
        return res.json({
          success: true,
          message: 'Update queued; TaskHunter will restart after installation completes',
          version: updateInfo.version,
          packageManager: pm,
          autoRestart: true,
      ...
      let restartCmdFallback = `taskhunter serve --port ${storedOptions.port}`;
      ...
      const updateLogPath = path.join(taskhunterDataDir, 'update-install.log');
      const logPreamble = [
        '',
        `=== TaskHunter update ${new Date().toISOString()} ===`,
      ...
      const windowsScriptPath = path.join(taskhunterDataDir, 'update-install.cmd');
      if (isWindows) {
        try {
      ...
      res.json({
        ...
        setTimeout(() => {
          ...
          : `
            ...
            if [ $? -eq 0 ]; then
              echo "Update successful, restarting TaskHunter..."
              ${restartCmd || 'echo "Service manager will restart TaskHunter."'}
            else
              echo "Update failed"
              exit 1
            fi
  ...
  app.get('/api/taskhunter/models-metadata', async (_req, res) => {
    try {
