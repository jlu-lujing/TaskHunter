...
the OpenCode process TaskHunter launches that reconnects those servers, so a
...
1. `prepareManagedOpenCodeEnv(configContent)` materializes the plugin under
   `<taskhunter-data-dir>/mcp-reconnect/` and appends its `file://` URL to
...
- External OpenCode (`OPENCODE_HOST` or skip-start) and VS Code's separate
   OpenCode lifecycle: not injected, because TaskHunter does not control that
  process environment.
