...
const materialize = async (rawConfig = '{}') => {
  const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'taskhunter-mcp-reconnect-'));
  ...
  const pluginPath = path.join(dataDir, 'mcp-reconnect', 'taskhunter-mcp-reconnect-plugin.js');
  const pluginModule = await import(`${pathToFileURL(pluginPath).href}?test=${Date.now()}-${Math.random()}`);
  return { prepared, pluginPath, plugin: pluginModule.TaskHunterMcpReconnectPlugin };
};
