...
import { TaskHunterControlError, asControlError } from '../taskhunter-control/error.js';
...
const waitForWorktreeBootstrapReady = async ({ directory }) => {
  ...
  for (;;) {
    ...
    if (status?.status === 'failed') {
      throw new TaskHunterControlError(`Worktree bootstrap failed: ${status.error || 'unknown error'}`, 500);
    }
    const phase = status?.phase;
    if (status?.status === 'ready' || phase === 'git-ready' || phase === 'setup-ready') return;
    if (Date.now() >= deadline) {
      throw new TaskHunterControlError('Timed out waiting for the worktree bootstrap', 500);
    }
...
export const createTaskHunterSessionService = (dependencies) => {
  const {
  ...
  const validateRequestedSelection = async ({ directory, requestedModel, requestedAgent, requestedVariant }) => {
    ...
    if (requestedAgent && agents.length > 0) {
      ...
      if (!agent) {
        throw new TaskHunterControlError(`Unknown agent '${requestedAgent}' for ${directory}`, 400);
      }
      if (!isPrimaryAgentMode(agent.mode)) {
        throw new TaskHunterControlError(`Agent '${requestedAgent}' is a subagent and cannot receive a prompt directly`, 400);
      }
    }
  ...
  const dispatchPrompt = async ({
    ...
    if (goalInput.enabled) {
      ...
      await (createSessionGoalOverride || createSessionGoal)({
        ...
        onWarning: (message, error) => console.warn(`[TaskHunterSessions] ${message}:`, error?.message || error),
      });
    }
  ...
  const archive = async (payload = {}) => {
    ...
    if (!parsed.ok) {
      throw new TaskHunterControlError(parsed.error, 400);
    }
    ...
    if (!resolvedDirectory.ok) {
      throw new TaskHunterControlError(resolvedDirectory.error, resolvedDirectory.status || 400);
    }
    ...
    for (const sessionID of parsed.ids) {
      try {
        ...
        console.warn('[TaskHunterSessions] failed to archive session', sessionID, error);
  ...
  const create = async (payload = {}) => {
    ...
    if (!goalInput.ok) {
      throw new TaskHunterControlError(goalInput.error, 400);
    }
  ...
  const runExisting = async (action, sourceSessionId, payload = {}) => {
    ...
    const prompt = asNonEmptyString(payload.prompt);
    if (!sourceSessionID) throw new TaskHunterControlError('sessionId is required', 400);
    if (!prompt) throw new TaskHunterControlError('prompt is required', 400);
    const goalInput = resolveGoalInput(payload, prompt);
    if (!goalInput.ok) throw new TaskHunterControlError(goalInput.error, 400);
...
export const registerTaskHunterSessionRoutes = (app, dependencies) => {
  const service = dependencies.sessionService || createTaskHunterSessionService(dependencies);

  app.post('/api/taskhunter/sessions', express.json({ limit: '1mb' }), async (req, res) => {
    try {
      return res.json(await service.create(req.body && typeof req.body === 'object' ? req.body : {}));
    } catch (error) {
      console.error('[TaskHunterSessions] failed to create session:', error);
      return sendServiceError(res, error, 'Failed to create session');
    }
  });

  app.post('/api/taskhunter/sessions/archive', express.json({ limit: '1mb' }), async (req, res) => {
    try {
      return res.json(await service.archive(req.body && typeof req.body === 'object' ? req.body : {}));
    } catch (error) {
      console.error('[TaskHunterSessions] failed to archive sessions:', error);
      return sendServiceError(res, error, 'Failed to archive sessions');
    }
  });

  app.post(
    '/api/taskhunter/sessions/:sessionId/send',
    express.json({ limit: '1mb' }),
    async (req, res) => {
      try {
        return res.json(await service.send(req.params.sessionId, req.body));
      } catch (error) {
        console.error('[TaskHunterSessions] failed to send session:', error);
        return sendServiceError(res, error, 'Failed to send session');
      }
    },
  );
  app.post(
    '/api/taskhunter/sessions/:sessionId/fork',
    express.json({ limit: '1mb' }),
    async (req, res) => {
      try {
        return res.json(await service.fork(req.params.sessionId, req.body));
      } catch (error) {
        console.error('[TaskHunterSessions] failed to fork session:', error);
