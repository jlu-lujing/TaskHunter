...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const inlineCommentThreads = new InlineCommentThreads({
    ...
    reportUndelivered: () => {
      vscode.window.showWarningMessage(t('TaskHunter [Add Comment]: The comment never reached the chat and was discarded'));
    },
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('taskhunter.addLineComment', () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('TaskHunter [Add Comment]: No active editor'));
        return;
      }
      ...
      if (!inlineCommentThreads.canCommentOn(editor.document.uri)) {
        vscode.window.showWarningMessage(t('TaskHunter [Add Comment]: File is outside the workspace'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('taskhunter.submitLineComment', async (reply: vscode.CommentReply) => {
      await inlineCommentThreads.submitReply(reply);
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('taskhunter.removeLineComment', (thread: vscode.CommentThread) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('taskhunter.internal.inlineCommentsSync', (message: { snapshot: unknown; surfaceId: string }) => {
