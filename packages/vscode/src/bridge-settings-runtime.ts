...
const writeSharedSettingsToDisk = async (changes: Record<string, unknown>): Promise<void> => {
  ...
  try {
    await fs.promises.mkdir(path.dirname(TASKHUNTER_SHARED_SETTINGS_PATH), { recursive: true });
    ...
    tmp = `${TASKHUNTER_SHARED_SETTINGS_PATH}.tmp-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    await fs.promises.writeFile(tmp, JSON.stringify(next, null, 2), 'utf8');
    await fs.promises.rename(tmp, TASKHUNTER_SHARED_SETTINGS_PATH);
  } catch {
    if (tmp) {
