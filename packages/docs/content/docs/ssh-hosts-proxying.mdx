...
TaskHunter can read hosts from your local SSH config, the same place used by commands like `ssh work-server`.
...
TaskHunter creates a remote instance using that SSH command. When it reaches **ready**, the remote TaskHunter UI opens in the desktop app.
...
TaskHunter supports three forward types:
...
Choose **Dynamic (-D)** when you want other tools on your computer to browse through the remote machine. TaskHunter opens a local SOCKS proxy port for that SSH connection.
...
- [Remote Instances](/remote-instances/) — connect the desktop app to TaskHunter on another machine
