---
title: Reverse proxy
description: Skonfiguruj TaskHunter poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz TaskHunter za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że TaskHunter działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli TaskHunter już kompresuje odpowiedzi.
...
- TaskHunter dostępny bezpośrednio w LAN
...
- TaskHunter kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez TaskHunter, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
