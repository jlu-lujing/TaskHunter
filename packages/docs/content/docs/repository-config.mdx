...
Project actions, worktree setup commands, and draft starters live in your own TaskHunter settings by default. Nobody else sees them. If you want a teammate who clones the repository to get the same dev server action and the same `bun install` on every new worktree, move those items into the repository.

TaskHunter stores them in `.taskhunter/project.json` at the repository root. The file appears only when you move the first item there, and it goes away again when you move the last one out. Commit it like any other file.
...
Only `version` is required. Every other key is optional, and TaskHunter writes only the keys that carry something.

`setupWorktree` is the list of shell commands TaskHunter runs inside a new worktree right after creating it, in order. Use `$ROOT_PROJECT_PATH` for the main checkout's path. `setupWorktreeWait: true` makes TaskHunter wait for these commands before it starts a session in the worktree.

`projectActions` is the list of actions in the header menu. `id`, `name`, and `command` are required. `icon` is optional and falls back to a play icon; the names TaskHunter knows are `play`, `build`, `lint`, `terminal`, `tools`, `bug`, `flask`, `rocket`, `code`, `server`, `branch`, `search`, `settings`, `brain`, `stack`, `robot`, `command`, and `file`. `autoOpenUrl: true` opens the address the command prints, see [Preview & Dev Servers](/preview/). `platforms` limits the action to `macos`, `linux`, or `windows`. `runIn: "parent"` runs the action in the main checkout instead of the current worktree.
...
`plansDir` is where repository plans live. Leave it out to use `.taskhunter/plans`. See below.
...
Setup commands and actions from the repository run on your machine, and a `git pull` can change them. So the first time one of them is about to run, TaskHunter shows the exact commands and asks. **Trust and run** remembers your answer on this instance. **Not this time** runs only your own commands.
...
Plans on the Plans tab can also live in the repository, as Markdown files. The folder is `.taskhunter/plans` by default. Set **Plans folder** in the project's settings to use another folder inside the repository, for example `docs/plans` if your team already keeps plans there. A custom folder replaces the default completely: TaskHunter reads and writes only that folder, so move existing files yourself when you change it.

Every `.md` file in that folder shows on the Plans tab, including files written by other tools. Editing one in TaskHunter saves the file as you typed it. A plan you move into the repository keeps its identity, so sessions that had it attached still find it.
