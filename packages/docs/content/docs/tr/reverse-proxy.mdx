---
title: Reverse Proxy
description: TaskHunter'ı Nginx, Nginx Proxy Manager veya başka bir reverse proxy arkasında doğru şekilde ayarlayın.
---

# Reverse Proxy

TaskHunter'ı Nginx, Nginx Proxy Manager, Caddy, Cloudflare ya da başka bir reverse proxy arkasında çalıştırıyorsanız bu sayfayı kullanın.

## Proxy'lemeden önce

1. Önce TaskHunter'ın doğrudan çalıştığını doğrulayın.
...
- TaskHunter yanıtları zaten sıkıştırıyorsa proxy'de gzip'i kapatın.
...
- TaskHunter LAN üzerinden doğrudan erişilebilir
...
- TaskHunter HTTP yanıtlarını gzip ile sıkıştırır, eşik 1 KB'dir.
...
SSE akış rotaları TaskHunter tarafından sıkıştırmanın dışında tutulur, ancak CDN bunları yine de buffer'layabilir. SSE yollarında buffer'lamayı nasıl kapatacağınızı CDN belgelerinden kontrol edin.
