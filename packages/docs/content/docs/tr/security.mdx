...
TaskHunter makinenize ve kodunuza erişim verir. Bu yüzden sizden başkası ulaşmadan önce kilitleyin. Bu sayfa arayüz parolasını, passkey'leri ve TaskHunter'ı bir ağa açmadan önce bilmeniz gerekenleri anlatır.

## Arayüz parolası ayarlama

TaskHunter'ı bir parola ile başlatın, tarayıcı arayüzü bunu ister:

```bash
taskhunter --ui-password be-creative-here
```

Parolayı komut satırına yazmak yerine `TASKHUNTER_UI_PASSWORD` ortam değişkeniyle de ayarlayabilirsiniz. Oturum açtıktan sonra TaskHunter bir süre cihazı hatırlar, böylece her seferinde sorulmaz.
...
Parola ayarlandıktan sonra daha hızlı giriş için passkey ekleyebilirsiniz. Bunlar Face ID, Touch ID ya da bir güvenlik anahtarı olabilir. **Ayarlar → TaskHunter → Passkeys** bölümünden ekleyin.
...
- Varsayılan olarak TaskHunter yalnızca kendi makinenizde dinler (`127.0.0.1`). Daha geniş dinlemek için bilinçli bir değişiklik gerekir ve önce parola ayarlamalısınız.
...
- TaskHunter'ı kendi HTTPS sunucunuzun arkasına koyuyorsanız [Reverse Proxy](/reverse-proxy/) sayfasına bakın.
...
- [Reverse Proxy](/reverse-proxy/) — TaskHunter'ı kendi sunucunuzun arkasında çalıştırma
