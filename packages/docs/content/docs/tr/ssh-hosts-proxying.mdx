...
TaskHunter, yerel SSH yapılandırmanızdaki host'ları okuyabilir. Bu, `ssh work-server` gibi komutların kullandığı yerdir.
...
TaskHunter o SSH komutunu kullanarak bir uzak örnek oluşturur. **ready** noktasına geldiğinde uzak TaskHunter arayüzü masaüstü uygulamasında açılır.
...
TaskHunter üç yönlendirme tipini destekler:
...
Bilgisayarınızdaki diğer araçların uzak makine üzerinden gezmesini istiyorsanız **Dynamic (-D)** seçin. TaskHunter, o SSH bağlantısı için yerel bir SOCKS proxy portu açar.
...
- [Uzak örnekler](/remote-instances/) — masaüstü uygulamasını başka bir makinedeki TaskHunter'a bağlayın
