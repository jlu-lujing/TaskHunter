...
description: Tünelleri, uzak instance'ları ve TaskHunter'a başka bir cihazdan erişmeyi düzeltin.
---

# Uzaktan Erişim

TaskHunter'a telefonunuzdan ya da başka bir makineden ulaşamıyorsanız, çözüm bağlantı biçiminize göre değişir.
...
- sunucunun çalıştığını `taskhunter status` ile doğrulayın
...
- **Anywhere** eşleştirmesi için sunucudaki **Settings → Remote Instances → TaskHunter Relay** ayarını kontrol edin. **Connected** yazmalı. Değilse devre dışı bırakıp yeniden etkinleştirin
...
- `taskhunter tunnel status --all` çalıştırın
...
Bir [remote instance](/remote-instances/) takılırsa, TaskHunter hangi adımın başarısız olduğunu söyler:

- **auth** — SSH ya da UI parolanız reddedildi. Tekrar girin
- **install / start** — TaskHunter uzak makinede sunucuyu kuramadı ya da başlatamadı. O makinenin gereksinimlerini kontrol edin
- **forwarding** — bağlantı kuruludur ama port size ulaşmıyordur. Farklı bir local port deneyin

## Kendi sunucunuzun arkasında

TaskHunter'ı bir reverse proxy arkasına koyduysanız ve garip açılıyorsa ya da bağlanmıyorsa, [Reverse Proxy](/reverse-proxy/) bölümüne bakın.
