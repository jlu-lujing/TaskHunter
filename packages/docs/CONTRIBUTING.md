...
This package is docs content source-of-truth for TaskHunter.
...
- Lead with the task, not background. The first line should say what the page is
  for ("Use `taskhunter tunnel` to expose a running TaskHunter instance.").
...
- A basic page should fit in a screen or two. Long, dense reference pages (like
  ...
  this page if you run TaskHunter behind...").
...
```mdx
3. Run `taskhunter --ui-password be-creative-here`.
4. Open the printed URL (usually `http://localhost:3000`).

You should land on the TaskHunter session list. If you see it, the server is
running.
```
...
- Explain a term the first time it appears, in parentheses, in everyday words:
  - good: start a tunnel (a public link to your local TaskHunter)
...
2. Add frontmatter at top:
   ...
   ---
   title: Remote Access
   description: Access TaskHunter from outside your local network.
   ---
   ```
...
The docs are translated into the same languages the TaskHunter app ships in.
...
`taskhunter-website/apps/docs/astro.config.mjs` (`locales`). If a language is
...
---
title: Встановлення
description: Встановіть TaskHunter для десктопа, вебу або VS Code.
---
```
...
- brand and product nouns: TaskHunter, OpenCode, VS Code, PWA, GitHub, Discord,
  macOS, SSH
...
## Sync into taskhunter-website

`taskhunter-website` renders/deploys docs via Starlight in `apps/docs`.

After docs content updates here:

1. copy `packages/docs/content/docs/*` -> `taskhunter-website/apps/docs/src/content/docs/*`
   (this is recursive, so locale folders like `uk/` and `zh-cn/` carry over with
   no extra steps)
2. map `packages/docs/sidebar.config.json` into `taskhunter-website/apps/docs/astro.config.mjs` sidebar
