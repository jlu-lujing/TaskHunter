...
Rendering and hosting still happen in `taskhunter-website` (`apps/docs`).
...
- creates `taskhunter-docs-source-<sha>.tar.gz`
...
to `taskhunter-website` so it re-syncs and redeploys the docs. This fires on
...
- `source_repo`
- `source_ref` — the ref the website checks out from `taskhunter` (`main` on a
  push, the tag on a release)
- `archive_name`

`taskhunter-website`'s `deploy-docs.yml` listens for this event
(`repository_dispatch: types: [docs_source_updated]`), checks out
`taskhunter` at `source_ref`, runs `docs:sync`, builds `apps/docs`, and deploys
to Cloudflare Pages. That repo needs its own secrets: `TASKHUNTER_REPO_TOKEN`
