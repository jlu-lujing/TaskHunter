...
const localOrigin = readArgValue('--taskhunter-local-origin');
const apiBaseUrl = readArgValue('--taskhunter-api-base-url');
const clientToken = readArgValue('--taskhunter-client-token');
const runtimeHeadersRaw = readArgValue('--taskhunter-runtime-headers');
const homeDirectory = readArgValue('--taskhunter-home');
const macosMajorRaw = readArgValue('--taskhunter-macos-major');
const macosMajor = Number.parseInt(macosMajorRaw, 10);
const trayEnabled = process.platform !== 'darwin' || readArgValue('--taskhunter-tray-enabled') !== '0';
...
//  - __TASKHUNTER_ELECTRON__ is a shell-identity flag (no capability).
...
//  - __TASKHUNTER_DESKTOP__ is the IPC channel to the main process. It is
...
const isLocalPage = currentOrigin !== 'null'
  && (currentOrigin === 'taskhunter-ui://app'
  || (localOrigin && currentOrigin === localOrigin));

// Remote pages need __TASKHUNTER_LOCAL_ORIGIN__ so the HostSwitcher knows
...
if (localOrigin) {
  contextBridge.exposeInMainWorld('__TASKHUNTER_LOCAL_ORIGIN__', localOrigin);
}

if (apiBaseUrl) {
  contextBridge.exposeInMainWorld('__TASKHUNTER_API_BASE_URL__', apiBaseUrl);
}

if (clientToken && isLocalPage) {
  contextBridge.exposeInMainWorld('__TASKHUNTER_CLIENT_TOKEN__', clientToken);
}
...
const relayHostId = readArgValue('--taskhunter-relay-host-id');
if (relayHostId && isLocalPage) {
  contextBridge.exposeInMainWorld('__TASKHUNTER_RELAY_HOST_ID__', relayHostId);
}
...
if (runtimeHeadersRaw && isLocalPage) {
  try {
    ...
    if (runtimeHeaders && typeof runtimeHeaders === 'object') {
      contextBridge.exposeInMainWorld('__TASKHUNTER_RUNTIME_HEADERS__', runtimeHeaders);
    }
  } catch {
  }
}
...
if (isLocalPage && homeDirectory) {
  contextBridge.exposeInMainWorld('__TASKHUNTER_HOME__', homeDirectory);
}
...
if (Number.isFinite(macosMajor) && macosMajor > 0) {
  contextBridge.exposeInMainWorld('__TASKHUNTER_MACOS_MAJOR__', macosMajor);
}

contextBridge.exposeInMainWorld('__TASKHUNTER_ELECTRON__', {
  runtime: 'electron',
  arch: process.arch,
  trayEnabled,
});

contextBridge.exposeInMainWorld('__TASKHUNTER_PLATFORM__', process.platform);
...
ipcRenderer.on('taskhunter:emit', (_evt, payload) => {
