...
//
// Surfaces a glanceable, always-visible view of TaskHunter's live state:
...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'TaskHunter — no active sessions';
  const bits = [];
  ...
  return `TaskHunter — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    if (isLinux || process.platform === 'win32') {
      ...
      template.push(
        { label: 'Show TaskHunter', click: () => onAction({ type: 'show-main-window' }) },
        { type: 'separator' },
        { label: 'Quit TaskHunter', click: () => onAction({ type: 'quit' }) },
      );
    }
